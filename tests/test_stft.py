...
def test_stft_istft_consistency(
    ...
    if same_length:
        np.testing.assert_allclose(
            stft.inverse(stft(x), len(x))[seg].numpy(), x[seg].numpy(), atol=5e-04
        )
        x2 = torch.stack([x, x], dim=0)
        np.testing.assert_allclose(
            stft.inverse(stft(x2), len(x))[:, seg].numpy(),
            x2[:, seg].numpy(),
            atol=5e-04,
        )
    else:
        ...
        np.testing.assert_allclose(
            x3[seg].numpy(), x[: len(x3)][seg].numpy(), atol=5e-04
        )
        x2 = torch.stack([x, x], dim=0)
        x3 = stft.inverse(stft(x2), len(x))
        np.testing.assert_allclose(
            x3[:, seg].numpy(), x2[:, : x3.size(1)][:, seg].numpy(), atol=5e-04
        )
...
def test_stft_consistency(
    ...
    for s in specs[1:]:
        np.testing.assert_allclose(specs[0].numpy(), s.numpy(), atol=1e-06)
